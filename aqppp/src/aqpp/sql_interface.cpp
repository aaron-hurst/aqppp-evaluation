...
void SqlInterface::ShowError(unsigned int handle_type,
                             const SQLHANDLE &handle) {
  SQLWCHAR sqlstate[1024];
  SQLWCHAR message[1024];
  if (SQL_SUCCESS == SQLGetDiagRec(handle_type, handle, 1, sqlstate, NULL,
                                   message, 1024, NULL))
...
void SqlInterface::ShowError(unsigned int handle_type, const SQLHANDLE &handle,
  ...
  std::cout << "Error occurred while executing SQL query: " << query_str
            << std::endl;
...
// categorical
const SqlInterface::TableColumns SqlInterface::GetTableColumns(
    SQLHANDLE &sqlConnectionHandle, std::string table_name) {
  ...
  if (SQL_SUCCEEDED(rc)) {
    ...
    if (field_count == 2) {
      // Loop through the rows in the result set and parse into TableColumns
      // results
      rc = SQLFetch(h_stmt);
      SQLLEN ret1;
      SQLLEN ret2;
      ...
      while (SQL_SUCCEEDED(rc)) {
        ...
        if (std::find(std::begin(numeric_types), std::end(numeric_types),
                      column_type) != std::end(numeric_types)) {
      ...
      std::cout << "Error: invalid number of fields returned: " << field_count
                << std::endl;
    }
  } else {
    std::cout << "error" << std::endl;
...
std::string SqlInterface::ComputeRandStr(SQLHANDLE &sqlConnectionHandle,
  ...
  return "RAND(BINARY_CHECKSUM(" + numeric_part + catagory_part +
...
double SqlInterface::Column2Numeric(SQLHANDLE &sqlstatementhandle, int col_id,
                                    std::string col_name) {
  const int max_char_len = 300;
  transform(col_name.begin(), col_name.end(), col_name.begin(),
  ...
  if (col_name.find("time") != std::string::npos)  // time columns
  {
    double data = -1;
    TIMESTAMP_STRUCT ts;
    SQLGetData(sqlstatementhandle, col_id + 1, SQL_C_TYPE_TIMESTAMP, &ts, 0,
               NULL);
  ...
  float data_float = 0;
  RETCODE rc;
  rc = SQLGetData(sqlstatementhandle, col_id + 1, SQL_C_FLOAT, &data_float, 0,
                  NULL);
  if (!SQL_SUCCEEDED(rc)) {
    std::cout << "Error getting data in SqlInterface::Column2Numeric."
              << std::endl;
    ShowError(SQL_HANDLE_STMT, sqlstatementhandle);
  }
...
std::string SqlInterface::ReadTableStr(
    std::string db_name, std::string table_name, std::string AGGREGATE_NAME,
