...
double QueryRealValue(
  ...
  if (SQLAllocHandle(SQL_HANDLE_STMT, sqlconnectionhandle,
    ...
    return -1;
  if (CLEANCACHE)
    aqppp::SqlInterface::SqlQuery("DBCC DROPCLEANBUFFERS;", sqlstatementhandle);
  std::string bk = "";
  ...
  for (int ci = 0; ci < DIM; ci++) {
    ...
    if (id >= 0) {
      ...
      if (PAR.CONDITION_NAMES[ci] == "pickup_time" ||
          PAR.CONDITION_NAMES[ci] == "dropoff_time") {
    ...
    lb_str = "'" + lb_str + "'";  //' is for catagory data and date. It is also
                                  //works for numeric data.
    ub_str = "'" + ub_str + "'";
    bk += PAR.CONDITION_NAMES[ci] + ">=" + lb_str + " AND " +
          PAR.CONDITION_NAMES[ci] + "<=" + ub_str;
  }
  std::string s = "select " + agg_action + "(" + PAR.AGGREGATE_NAME +
                  ") from " + PAR.DB_NAME + "." + table_name + " where " + bk +
                  ";";
  std::cout << s << std::endl;
...
double QueryRealValue(const std::vector<aqppp::Condition>& cur_q,
  ...
  return QueryRealValue(cur_q, table_name, sqlconnectionhandle, PAR, agg_action,
                        distinct_itos, CLEANCACHE);
}
